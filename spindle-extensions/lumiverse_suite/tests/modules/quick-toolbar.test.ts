...
function harness(value: unknown, failures: { mount?: boolean; update?: boolean } = {}) {
  ...
  const context = {
    ...
    settings: { get: async () => value, set: async (_key: string, next: unknown) => { value = next }, remove: async () => undefined, watch: (_key: string, listener: (next: unknown) => void) => { watch = listener; return () => { watch = undefined } }, core: { get: () => undefined, watch: () => () => undefined, list: () => [] } },
  } as never
